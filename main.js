...
// Auto-updater
// ---------------------------------------------------------------------------
// On Windows, electron-updater's sha512 check is brittle: Defender frequently
// touches the downloaded exe, breaking the hash even though the file is fine.
// We use electron-updater only to *detect* a new version, then download the
// installer ourselves via plain HTTPS and run it — no hash verification.
...
async function _downloadInstallerWindows(info) {
  ...
  // Clean stale installers from prior versions
  try {
  ...
  // Re-use already-downloaded installer if it looks intact
  try {
    ...
    if (expectedSize > 0 && st.size === expectedSize) {
      ...
      _broadcast('update-downloaded', info);
      return;
    }
  ...
  await new Promise((resolve, reject) => {
    ...
    _httpsGetFollow(url, (res) => {
      ...
      res.on('data', (chunk) => {
        ...
        if (now - lastEmit > 200) {
          ...
          _broadcast('update-progress', _updateProgressInfo);
        }
      });
      res.pipe(file);
    ...
    _broadcast('update-error', err.message || String(err));
...
autoUpdater.on('error', (err) => {
  ...
  // On Windows we run our own download — electron-updater errors (incl. the
  // sha512 mismatch) are advisory only as long as our manual flow is healthy.
  if (IS_WIN && (_manualDownloadActive || _manualInstallerPath)) return;
  _broadcast('update-error', msg);
  _updateAvailableInfo = null;
  _updateProgressInfo  = null;
  setTimeout(() => checkForUpdatesIfEnabled(), 60_000);
});
...
ipcMain.on('install-update-now', () => {
  ...
  if (IS_WIN && _manualInstallerPath && fs.existsSync(_manualInstallerPath)) {
    // /S = silent NSIS install, --force-run relaunches Privoo afterwards
